    ...
    return (
        <div className="w-full shadow-md relative">
            <div ref={dropBox} className="relative w-full">
                {showDropDown ? <DropDownBox show={setShowDropDown} setDest={setTo} dest={dest} /> : null}
            </div>
            ...
            <input type="text" id="search" className="bg-gray-50 border-2 border-gray-50 rounded-lg text-gray-900 text-sm focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-300 block w-full pl-10 px-2.5 py-3" placeholder="Where to?" required readOnly value={dest} onClick={(e) => setShowDropDown(!showDropDown)} />
        </div>
    )
}